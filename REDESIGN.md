...
- **Phase 5 — Auth + Settings.** Two sub-phases. **Decision (2026-04-13): use deep-link (`schemagic://auth?token=...`), not httpOnly cookies** — the Tauri shell ships as `STATIC_EXPORT=1` with no API routes, so cookies set on `schemagic.design` don't exist inside the Tauri webview. Deep link hands the identity JWT back into the app.
  ...
  - **5b — Auth + Settings UI.** ✅ Done (2026-04-13, verified in Tauri app). Email page has `form`/`sent` state machine, magic-link call via `requestMagicLink`, dev-only escape hatch under form. Paywall matches Figma `78:4` (single card, "FREE TIER USED" header in accent, "$5 USD/month" copy — note: Figma says AUD but code stays USD to match Stripe price). Settings Preferences stubs now read "Coming soon". **Scope creep**: had to add `shell:allow-open` to `tauri/capabilities/default.json` — without it `@tauri-apps/plugin-shell` `open()` is denied so the Subscribe CTA silently no-ops. Technically Rust/config, but paywall is non-functional without it. Copy below is the original UI-only plan:
